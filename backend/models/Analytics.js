...
class Analytics {
    ...
    // Submission Trends (Unified DSA + Frontend)
            ...
            const query = `
                ...
                FROM (
                    ...
                    SELECT 
                        DATE(created_at) as date,
                        COUNT(*)::int as total,
                        COUNT(CASE WHEN status = 'completed' THEN 1 END)::int as accepted,
                        0 as failed -- Frontend projects don't have a "failed" state usually
    ...
    // Top Performers (Unified DSA + Frontend)
        ...
        try {
            const query = `
                SELECT 
                    ...
                    u.is_prime,
                    COALESCE(dsa.problems_solved, 0)::int as problems_solved,
                    COALESCE(fe.projects_completed, 0)::int as projects_completed,
                    (COALESCE(dsa.problems_solved, 0) + COALESCE(fe.projects_completed, 0) * 5)::int as total_activity_score
                FROM users u
                LEFT JOIN (
                    ...
                    WHERE status = 'solved'
                    GROUP BY user_id
                ) dsa ON u.id = dsa.user_id
                LEFT JOIN (
                    SELECT user_id, COUNT(DISTINCT project_id) as projects_completed
                    FROM frontend_submissions
                    WHERE status = 'completed'
                    GROUP BY user_id
                ) fe ON u.id = fe.user_id
                ORDER BY total_activity_score DESC, problems_solved DESC
                LIMIT $1
            `;
            const result = await pool.query(query, [limit]);
            return result.rows;
    ...
    // Hourly Activity Pattern (Unified)
    ...
    // Submission Status Distribution (Unified)
        ...
        try {
            const query = `
                ...
                FROM (
                    ...
                    SELECT 
                        CASE WHEN status = 'completed' THEN 'Accepted' ELSE 'In Progress' END as status,
                        COUNT(*)::int as count
                    FROM frontend_submissions
                    GROUP BY status
                ) as unified
    ...
    // Get user profile for admin viewing (Unified)
        ...
        try {
            const userQuery = `
                SELECT 
                    ...
                    COALESCE(dsa.problems_solved, 0)::int as problems_solved,
                    COALESCE(fe.projects_completed, 0)::int as projects_completed,
                    COALESCE(s.total_submissions, 0)::int as total_submissions
                FROM users u
                LEFT JOIN (
                    SELECT user_id, COUNT(DISTINCT problem_id) as problems_solved
                    FROM user_problem_status
            ...
            // Get recent DSA submissions
            const dsaSubmissionsQuery = `
                SELECT 
                  s.status,
                ...
                LIMIT 5
            `;
            const dsaResult = await pool.query(dsaSubmissionsQuery, [userId]);

            // Get recent Frontend submissions
            const feSubmissionsQuery = `
                SELECT 
                  s.status,
                  s.created_at as date,
                  'HTML/CSS/JS' as language,
                  p.title as title,
                  'Frontend' as type
                FROM frontend_submissions s
                JOIN frontend_projects p ON s.project_id = p.id
                WHERE s.user_id = $1
                ORDER BY s.created_at DESC
                LIMIT 5
            `;
            const feResult = await pool.query(feSubmissionsQuery, [userId]);

            // Combine and sort
            const unifiedSubmissions = [...dsaResult.rows, ...feResult.rows]
                .sort((a, b) => new Date(b.date) - new Date(a.date))
                .slice(0, 10);

            user.recent_submissions = unifiedSubmissions;
